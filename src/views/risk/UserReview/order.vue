  ...
  <div>
    ...
    <div class="content">
      ...
      >
        ...
        <el-table-column v-if="search.orderType == 1" label="担保人信息">
          <template slot-scope="scope">
            <div>
        ...
        <el-table-column v-if="search.orderType == 1" label="职业">
          <template slot-scope="scope">
            <div>{{ OCCUPATION_STATUS[scope.row.careerCode] }}</div>
          </template>
        </el-table-column>

        <el-table-column label="用车城市">
          <template slot-scope="scope">
            <div>
              {{ scope.row.careerCode === 7 ? scope.row.city : "全部" }}
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="modelNamesStr" label="大数据查询">
        </el-table-column>

        <el-table-column prop="createdTime" label="申请时间"> </el-table-column>
          ...
          <template slot-scope="scope">
            <div>
              <el-button
                class="table-btn"
                type="text"
                @click="goDetailPage(scope.row.id, scope.row.auditRate, 1)"
                >查看详情</el-button
              >
            </div>
            <div>
              <el-button
                class="table-btn"
                type="text"
                @click="goOrderDetailPage(scope.row)"
                >查看订单</el-button
              >
            </div>
            <div v-if="scope.row.queryStatus == 1">
              <div
                v-if="
                  scope.row.riskResult == 3 ||
                  scope.row.riskResult == 4 ||
                  scope.row.riskResult == 7
                "
              >
                <el-button
                  class="table-btn"
                  type="text"
                  @click="goDetailPage(scope.row.id, scope.row.auditRate, 1, 2)"
                >
                  人工审核
                </el-button>
              </div>
...
export default {
  ...
  methods: {
    ...
    selectRiskListByPage() {
      selectRiskListByPage(this.search)
        .then((res) => {
          if (res.code == 0) {
    ...
    selectRentalRiskList() {
      selectRentalRiskList(this.search)
        .then((res) => {
          if (res.code == 0) {
            this.list = res.data.records;
            this.total = res.data.total;
    ...
    getRiskResult(riskResult) {
      ...
      if (riskResult) {
        const item = CHECKED_STATUS_ARRAY.find(
          (item) => item.value == riskResult
        );
        if (item) {
          res = item.label;
        }
