  ...
  <div>
    <div class="content">
      ...
      >
        ...
        <el-table-column label="订单来源">
          <template slot-scope="scope">
            <div>
              <div>
                {{
                  scope.row.partnerRecommend === 1 ? "合伙人引荐" : "客户自购"
                }}
              </div>
              <div v-if="scope.row.partnerRecommend === 1">
                佣金:￥{{
                  formatThousandNumber(scope.row.distributionBrokerageFree)
                }}
              </div>
            </div>
          </template>
        ...
        <el-table-column prop="createdTime" label="下单日期" align="center">
        </el-table-column>
        ...
        <el-table-column label="订单金额">
          <template slot-scope="scope">
            <div>
              <div>
                车辆押金：{{ formatThousandNumber(scope.row.deposit) }}元
              </div>
              <div>
                订阅费用：{{ formatThousandNumber(scope.row.totalPayment) }}元
              </div>
              <div>
                买断价格：{{ formatThousandNumber(scope.row.buyoutsFee) }}元
              </div>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="110" align="center">
          <template slot-scope="scope">
            <div class="table-btn" @click="goOrderDetailPage(scope.row)">
              查看详情
            </div>
            <div
              v-if="scope.row.status == 16 || scope.row.status == 17"
              class="table-btn"
            ...
            <div
              v-if="
                scope.row.isEntrustOrder === 1 &&
                (scope.row.status == 10 || scope.row.status == 1)
              "
              class="table-btn"
              @click="setOrder(2, scope.row)"
            >
              编辑订单
            </div>
            <div
              ...
              class="table-btn"
              @click="deleteMewCarOrder(scope.row)"
            >
              删除订单
            </div>
    ...
    <el-dialog :visible.sync="showChooseDialog" width="150px">
    ...
    >
      ...
      >
        <el-table-column label="选择" align="center">
          <template slot-scope="scope">
            <div>
              <el-radio v-model="chooseBindCarId" :label="scope.row.id"
        ...
        <el-table-column label="实表里程数" align="center">
          <template slot-scope="scope">
            <div>
              {{
                scope.row.mileage
                  ? `${formatThousandNumber(scope.row.mileage)} 公里`
                  : "-"
              }}
            </div>
          </template>
        ...
        <el-table-column label="车辆增配项">
          <template slot-scope="scope">
            <div v-if="scope.row.carOtherConfigList.length > 3">
              <el-tooltip
                class="item"
                effect="dark"
                :content="scope.row.carOtherConfigList.join('\n')"
              >
                <div>
                  <div
                    v-for="(item, index) in scope.row.carOtherConfigList.slice(
                      0,
                      3
                    )"
                    :key="index"
            ...
            <div v-else>
              <div
                v-for="(item, index) in scope.row.carOtherConfigList"
                :key="index"
              >
                {{ item }}
              </div>
    ...
    >
      <div class="car-info">
        <el-image
          ...
          fit="cover"
        ></el-image>
        {{ currentOrder.carBrand }}·{{ currentOrder.carType }}
        {{ currentOrder.carTypeYear }} {{ currentOrder.carTypeYearProduct }}
      </div>
    ...
    <el-dialog title="提车信息" :visible.sync="showGetCarDialog" width="480px">
      <div class="car-info">
        <el-image
          style="width: 108px; height: 80px"
          :src="currentOrder.imageUrl"
...
import {
  ...
  bindCarDeliveryToOrder,
  noBindCarList,
  bindCarToOrder,
  createCarAndBindOrder,
  updateCarDeliveryTime,
  deleteMewCarOrder,
