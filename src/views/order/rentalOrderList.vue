  ...
  <div>
    ...
    <div class="content">
      ...
      >
        ...
        <el-table-column prop="cityName" label="用车城市"> </el-table-column>
        ...
        <el-table-column label="订单金额">
          <template slot-scope="scope">
            <div>
              <div>
                租车费用：{{ formatThousandNumber(scope.row.totalPayment) }}元
              </div>
              <div>
                租车押金：{{ formatThousandNumber(scope.row.deposit) }}元
              </div>
        ...
        <el-table-column label="订单状态">
          <template slot-scope="scope">
            <div :class="{ red: scope.row.status == 120 }">
              {{ CAR_RENTAL_ORDER_STATUS[scope.row.status] }}
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="110" align="center">
          <template slot-scope="scope">
            ...
            <div
              ...
              class="table-btn"
              @click="deleteCarOrder(scope.row)"
            >
              删除订单
            </div>
            <div v-if="scope.row.status == 70 || scope.row.status == 80">
              <div class="table-btn" @click="bindCar(scope.row)">
                {{ scope.row.carPlateNum ? "换绑车辆" : "绑定车辆" }}
              </div>
              <div class="table-btn" @click="setPickCarPerson(1, scope.row)">
                交车管理
              </div>
            </div>
            <div
              v-if="scope.row.status == 100 || scope.row.status == 110"
              class="table-btn"
              @click="setPickCarPerson(2, scope.row)"
            >
              取车管理
            </div>
            <div
              v-if="scope.row.status >= 130 && scope.row.vehicleDamage"
              class="table-btn"
              @click="handleVehicleDamage(scope.row)"
            >
              车损处理
            </div>
          </template>
...
<script>
import {
  selectRentalOrderList,
  bindRentalOrderCar,
  setPickCarPerson,
  deleteLongRentalCarOrder,
