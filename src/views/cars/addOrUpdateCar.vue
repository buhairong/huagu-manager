<template>
    <div>
        <el-breadcrumb separator="/">
            ...
            <el-breadcrumb-item class="subscription-plan">{{type === 'add' ? '添加' : '编辑'}}车辆</el-breadcrumb-item>
        </el-breadcrumb>
...
export default {
    ...
    mounted() {
        ...
        const newOrOld = this.$route.query.newOrOlder
        
        if (this.type === 'add') {
            this.detail = {
                ...detailInfo,
                newOrOld: this.$route.query.newOrOlder,
            }
        } else {
            this.getDetail(this.$route.query.carId, newOrOld)
        }
    },

    methods: {
        getDetail(carId,newOrOld) {
            const data = {
                id: carId,
                newOrOld,
            }
        ...
        next() {
            ...
            if (this.activeStep === 1) {
                if (this.detail.travelLicenseUrl || this.detail.carPlateNum || this.detail.carVin ||
                    this.detail.carEngineNum || this.detail.registerDate || this.detail.plateDate) {
        ...
        save() {
            ...
            for(const item of this.detail.insuranceList) {
                if (item.insuranceType == 0) {
                    if (item.insuranceNo || item.insuranceCompany || item.expireDate || item.insurancePicsList.length) {
