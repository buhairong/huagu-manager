<template>
    <div>
        <div class="page-wrap" v-loading="syncLoading" element-loading-text="车型同步中">
                ...
                <div class="products-wrap">
                    <div class="box box1">
                            ...
                            <div class="header">
                                ...
                                <div class="letter-wrap">
                                    <div 
                                        ...
                                        :key="item.letter"
                                        class="letter"
                                        :class="{'active-letter': item.letter === currentFilterLetter}"
                        ...
                        <div class="content-wrap" ref="letterBox" @scroll="handlerScroll">
                            <div 
                                ...
                                class="brand-wrap"
                            >
                                <div :ref="`letter${item.letter}`" class="letter" v-if="item.letter">{{item.letter}}</div>
                                <div 
                                    class="brand" 
                                    ...
                                    <div class="left">
                                        ...
                                        >
                                            <div 
                                                ...
                                                class="change-logo"
                                                @click="changeBrandLogo(item.id, item.brandName, item.logoUrl, item.logoUrlSmall)"
                                            >
                                                更换logo
                                            </div>
                                            <img class="logo" :src="item.logoUrl" v-if="item.logoUrl">
                                            <div
                                                v-else
                                                class="upload-logo" 
                                                @click="changeBrandLogo(item.id, item.brandName, item.logoUrl, item.logoUrlSmall)"
                    ...
                    <div class="box box2">
                        <div class="header-wrap">
                            <div class="header">
                                <div class="filter">
                                    <el-radio-group v-model="filter" @change="getFilterCarTypeList">
                                        <el-radio :label="2">全部车系</el-radio>
                                        <el-radio :label="1">已开放车系</el-radio>
                                        <el-radio :label="0">未开放车系</el-radio>
                                    </el-radio-group>
                                </div>
                                <div class="sync-type" @click="syncCarType('')">
                                    同步车型
                                </div>
                            </div>
                        </div>
                        <div class="content-wrap" ref="carTypeBox" @scroll="handlerCarTypeBoxScroll">
                            <el-empty v-if="carTypeErrorMsg" :description="carTypeErrorMsg"></el-empty>
                            <div v-else>
                                <div 
                                    class="car-type"
                                ...
                                >
                                    <div class="left">
                                        <template v-if="carType === 1">
                                            <div class="tip-icon">
                                                <i class="el-icon-warning-outline" v-if="item.fullData == 0"></i>
                                            </div>
                                            <div 
                                                class="car-img-wrap"
                                            ...
                                            >
                                                <div 
                                                    v-if="item.imageUrl.length && hoverCarTypeId === item.id"
                                                    class="change-car-img"
                                                    @click="changeCarImg(item.id, item.carType, item.imageUrl)"
                                                >
                                                    更换图片
                                                </div>
                                                <img class="car-img" :src="item.imageUrl[0]" v-if="item.imageUrl.length">
                                                <div
                                                    v-else
                                    ...
                                    <div class="right">
                                        <div class="text">{{item.open ? '关闭车系' : '开放车系'}}</div>
                                        <el-switch
                                            v-model="item.open"
                                        ...
                                        <div class="link-item" @click="changeCarType(item.id, item.carType, item.seriesTypeList)">
                                            车系类型
                                        </div>
                                        <div class="link-item" @click="viewCarTypesDialog(item.id, item.carType, item.open)">
                                            查看车型
                                        </div>
                                    </div>
                                </div>
...
<script>
import { getBrandList, getCarTypeList, openOrCloseCarType, syncCarType, judgeSpiderCarInfoCompleted } from '@/api/goods/goods'
...
export default {
    ...
    methods: {
        ...
        scrollToLetter(item) {
            ...
            const scrollTop = this.letters.find(letter => letter.letter === item.letter).scrollTop
        ...
        getFilterCarTypeList() {
            ...
            if (this.filter === 2) {
                ...
                this.filterCarTypeList = this.carTypeList.filter(item => item.open === this.filter)
            }
        },
        ...
        changeOpenStatus(e, carTypeId) {
            ...
            openOrCloseCarType(data).then((res) => {
                if (res.code == 0) {
                    this.getBrandList(this.currentFilterLetter)
                    this.getCarTypeList(this.currentBrandId, this.currentBrandName)
                } else {
